...
<body>
    ...
    <div>
      @foreach($products as $p)
        @php
          $change = $p['new'] - $p['old'];
          $sign = ''; $cls = '';
          if ($change > 0) { $sign = "+"; $cls="up"; }
          elseif ($change < 0) { $sign = "−"; $cls="down"; }
        @endphp
        <span>
          {{ $p['name'] }}: ₹{{ $p['new'] }}
  ...
  <section class="hero">
    <div class="hero-text">
      ...
      <a href="{{ route('landing') }}" class="btn-main">🚀 Get Started</a>
    </div>
    <img src="{{ asset('image/main.webp') }}" alt="Farm Image">
  </section>

  <section>
    <h2>About Us <span>Our Mission</span></h2>
    <div class="about">
      <img src="{{ asset('image/logo.png') }}" alt="Kisan Mitra Logo" onerror="this.style.display='none'">
      <p>
      ...
      <div style="margin-top:25px;">
        <a href="{{ route('apmc') }}" class="btn-main">➕ Add Member</a>
      </div>
    </div>
  </section>
  ...
  <footer>
    © 2025 Kisan Mitra | Designed with ❤️ for Farmers | <a href="{{ route('contact') }}">Contact Us</a>
  </footer>
